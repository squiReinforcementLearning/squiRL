...
class VPG_LE(pl.LightningModule):
    ...
    def __init__(self, hparams: argparse.Namespace) -> None:
        """Initializes VPG_LE class

        Args:
            hparams (argparse.Namespace): Stores all passed args
        """
        super(VPG_LE, self).__init__()
    ...
    def training_step(self, batch: Tuple[torch.Tensor, torch.Tensor],
        ...
        if torch.is_tensor(self.history):
            self.history = torch.cat((self.history, states[0]))
        else:
            self.history.append(states[0])
            self.history = torch.cat(self.history)

        if self.training_count % 50 == 0:
            lyapunov = [
                sum(nolds.lyap_e(self.history[:, i]))
                if len(self.history) > 20 else 0
                for i in range(self.history.shape[1])
            ]
            for i, j in enumerate(lyapunov):
                self.log('lyapunov' + str(i),
                         j,
                         on_step=False,
                         on_epoch=True,
                         prog_bar=True,
